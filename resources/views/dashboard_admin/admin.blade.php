...
<div class="modal-backdrop" id="modal-batch">
         ...
         <input type="hidden" id="batch-id" value="{{ $batch->uid ?? '' }}">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="batch-quota"
                        value="{{ $batch->kuota ?? 100 }}" min="0">
                </div>
    ...
    <div 
        id="batch-card-{{ $item->uid }}"
        class="batch-row"
        onclick="editBatchFromCard(this)"
        data-uid="{{ $item->uid }}"
        data-nama="{{ $item->nama_batch }}"
        data-tanggal-buka="{{ $item->tanggal_buka }}"
        data-tanggal-tutup="{{ $item->tanggal_tutup }}"
    ...
    >
        ...
        <div style="display:flex; align-items:center; gap:12px;">
            ...
            @if((int) $item->total_pendaftar === 0)
                <button type="button"
                    onclick="event.stopPropagation(); deleteBatch('{{ $item->uid }}')"
                    style="
                        border:none;
                        background:#fee2e2;
                        color:#dc2626;
        ...
        <div class="modal-backdrop" id="modal-logout">
            <div class="modal logout-modal">
                ...
                <div class="logout-actions">
                    ...
                    <button type="button" class="btn-logout-confirm" onclick="document.getElementById('logout-form').submit()">
                        Ya, Keluar
                    </button>
                </div>
            </div>
