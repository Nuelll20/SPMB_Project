...
<head>
    ...
    <style>
        ...
        * {
        ...
        .preview-title-divider::before,
        .preview-title-divider::after {
            content: '';
            flex: 1;
            height: 1px;
        ...
        .status-badge-document.pending-mode {
            background: var(--orange-bg);
            color: var(--orange);
            border: 1px solid rgba(245, 158, 11, 0.15);
        }

        .status-badge-document.valid-mode {
...
<body>
    ...
    <div class="container">
        ...
        <div class="admin-panel-card">
            ...
            <div class="meta-input-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="inNamaSiswa" class="admin-input"
                        value="{{ $pendaftaran->nama_siswa ?? '-' }}" oninput="executeMirroringLoop()" disabled>
                </div>
                <div class="form-group">
                    <label>Nomor Registrasi / No. Nota</label>
                    <input type="text" id="inNoReg" class="admin-input"
                        value="{{ $tagihan->nomor_tagihan ?? $pendaftaran->no_pendaftaran ?? '-' }}" oninput="executeMirroringLoop()" disabled>
                </div>
                <div class="form-group">
                    <label>Nama Orang Tua / Wali</label>
                    <input type="text" id="inNamaWali" class="admin-input" value="{{ $pendaftaran->nama_ortu ?? '-' }}"
                        oninput="executeMirroringLoop()" disabled>
                </div>
            </div>
            ...
            <table class="edit-table">
                ...
                <tbody id="adminTableBody">
                    @php
                        $defaultKomponen = collect([
                            (object) [
                                'nama_komponen' => 'Biaya Formulir & Administrasi Pendaftaran Gelombang Utama',
                                'qty' => 1,
                                'nominal' => 250000,
                            ],
                            (object) [
                                'nama_komponen' => 'Uang Pangkal / Uang Gedung (Sesuai Komitmen Awal)',
                                'qty' => 1,
                                'nominal' => 2500000,
                            ],
                        ]);

                        $items = isset($komponenTagihan) && $komponenTagihan->isNotEmpty()
                            ? $komponenTagihan
                            : $defaultKomponen;
                    @endphp
                    ...
                    @foreach ($items as $item)
                        <tr class="admin-item-row">
                            <td>
                                <input type="text" class="admin-input in-item-nama"
                                    value="{{ $item->nama_komponen }}"
                                    oninput="executeMirroringLoop()">
                            </td>
                            <td>
                                <input type="number" class="admin-input text-center-input in-item-qty"
                                    value="{{ $item->qty }}"
                                    oninput="executeMirroringLoop()">
                            </td>
                            <td>
                                <input type="number" class="admin-input text-right-input in-item-harga"
                                    value="{{ $item->nominal }}"
                                    oninput="executeMirroringLoop()">
                            </td>
                            <td style="text-align: center;">
                                <button type="button" class="btn-delete-row-action" onclick="deleteAdminRow(this)">
            ...
            <div class="meta-input-grid" style="margin-top: 8px;">
                <div class="form-group">
                    ...
                    <input type="number" id="inDiskon" class="admin-input"
                        value="{{ $tagihan->diskon_tagihan ?? 0 }}" oninput="executeMirroringLoop()">
                </div>
                <div class="form-group">
                    <label>Petugas Admin Verifikator</label>
                    <input type="text" id="inAdminPembuat" class="admin-input"
                        value="{{ $tagihan->dibuat_oleh ?? $pendaftaran->diverifikasi_oleh ?? session('email') ?? 'Admin JOG-WRO' }}"
                        oninput="executeMirroringLoop()">
                </div>
                <div class="form-group">
            ...
            <div class="admin-summary-area">
                <span style="font-size: 13.5px; font-weight: 700; color: var(--muted);"><i
                        class="fa-solid fa-circle-info"></i> Seluruh perubahan di atas langsung dirender ke dokumen di
                    bawah secara real-time.</span>
        ...
        <div class="print-document-wrapper">
            ...
            <div class="page page-break">
                ...
                <div class="content-block">
                    <div class="invoice-title-row">
                        ...
                        <span
                            style="font-family: 'DM Mono', monospace; font-size: 13.5px; font-weight: 700; color: var(--navy);"
                            id="viewNoReg">NO: -</span>
                    </div>
            ...
            <div class="page">
                ...
                <header class="authentic-header">
                    <div class="header-left">
                        <div class="header-logo-box" style="background: var(--blue);"><i
                ...
                <div class="content-block note-section">
                    <h3><i class="fa-solid fa-money-check-dollar" style="color: var(--blue);"></i> Rekening Virtual
                        Account (VA)</h3>

                    <div class="va-box">
                        <p style="font-size: 12px; font-weight: 800; color: var(--muted); text-transform: uppercase;">
                            Bank CIMB Niaga (Kode Bank: 022)</p>
                        <div class="va-number">9888 3471 2345 6789</div>
                        <p style="font-size: 12.5px; font-weight: 600; color: #334155; margin-top: 6px;">Atas Nama:
                            <strong style="color: var(--navy); text-transform: uppercase;" id="viewVaName">SAKTI
                                KANISIUS - BUDI SANTOSO</strong>
                        </p>
                    </div>

                    <h3><i class="fa-solid fa-list-check" style="color: var(--blue);"></i> Petunjuk Tatacara Transfer
                        Resmi</h3>

                    <ol class="step-list">
                        <li>Buka aplikasi <strong>Mobile Banking</strong>, Internet Banking, atau datangi mesin ATM
                            terdekat.</li>
                        <li>Pilih menu <strong>Transfer</strong>, kemudian pilih opsi <strong>Transfer ke Bank Lain /
                                CIMB Niaga</strong>.</li>
                        <li>Masukkan Kode Bank <strong>022</strong> diikuti Nomor Virtual Account:
                            <strong>9888347123456789</strong>.
                        </li>
                        <li>Pastikan pada layar konfirmasi tujuan muncul nama penerima yang valid: <strong
                                id="viewVaNameStep" style="text-transform: uppercase;">-</strong>.</li>
                        <li>Masukkan nominal transfer sama persis dengan angka tagihan lembar ke-1: <strong
                                id="viewTotalStep">-</strong>.</li>
                        <li>Simpan lembar PDF resmi ini sebagai bukti pembayaran digital otentik yang sah dari portal
                            admisi.</li>
                    </ol>

                    <div
                        style="margin-top: 50px; border-top: 1px solid var(--border); padding-top: 20px; font-size: 12px; color: var(--muted); line-height: 1.5; font-weight: 500;">
                        <p>* Dokumen ini dibuat otomatis oleh komputer <strong>Sistem Admisi Kanisius Terintegrasi
                                (SAKTI)</strong>. Diverifikasi sah oleh petugas resmi: <strong id="viewAdminSign"
                                style="color: var(--navy);">-</strong> pada tanggal {{ now()->format('d/m/Y') }}.</p>
                    </div>
                </div>
            </div>
    ...
    <div id="confirmBackModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <p class="modal-desc">Seluruh perubahan komponen invoice yang Anda ketik manual akan dibuang dan hilang dari
                memori workspace kerja.</p>
            <div class="modal-btn-group">
                <button type="button" onclick="closeConfirmBackModal()"
                    class="modal-btn modal-btn-cancel">Batal</button>
                <button type="button" onclick="executeBackNavigation()" class="modal-btn modal-btn-confirm">Ya,
                    Keluar</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function simulateSaveAndPrint() {
            ...
            rows.forEach(row => {
                const namaKomponen = row.querySelector('.in-item-nama').value.trim();
                const qty = parseFloat(row.querySelector('.in-item-qty').value) || 0;
                const nominal = parseFloat(row.querySelector('.in-item-harga').value) || 0;
            ...
            if (items.length === 0) {
                showToast('❌ Minimal harus ada 1 komponen biaya.');
                return;
            }
            ...
            showToast('<i class="fa-solid fa-floppy-disk"></i> Menyimpan tagihan ke database...');

            fetch(saveTagihanUrl, {
                method: 'POST',
                headers: {
                ...
                .then(async response => {
                    ...
                    if (!response.ok || !data.success) {
                        throw new Error(data.message || 'Tagihan gagal disimpan.');
                    }

                    showToast('✅ Tagihan berhasil disimpan. Membuka cetak PDF...');
